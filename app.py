...
def init_app():
    ...
    app.config["SECURITY_PASSWORD_SALT"] = ENVIRONMENT.get(
...
def create_default_user():
    if len(User.query.all()) == 0:
        ...
        db.session.commit()
        logging.info(
            "Initial user created! Credentials: admin@localhost/admin ")


def init_proxy_settings():
    if len(Settings.query.all()) == 0:
        proxy_settings = [(setting[0].lstrip("SQUID_").lower(), setting[1])
                          for setting in ENVIRONMENT.items() if setting[0].startswith("SQUID_")]
